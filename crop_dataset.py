...
def crop_dataset(source_dir, dest_dir, max_images=None):
    """
    Crop images from source directory and save them to destination directory.
    
    Args:
        source_dir (str): Path to source directory containing images
        dest_dir (str): Path to destination directory for cropped images
        max_images (int, optional): Maximum number of images to process. Defaults to None (process all).
    """
...
if __name__ == "__main__":
    # Example usage
    source_dir = "driving_dataset_4"
    dest_dir = "driving_dataset_4_cropped"
    crop_dataset(source_dir, dest_dir)
